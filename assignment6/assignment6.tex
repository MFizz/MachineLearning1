...
\begin{figure}[htb]
\includegraphics[trim= 0cm 10cm 0cm 10cm,scale=0.6]{prob_dist_w1.pdf}
...
\begin{figure}[htb]
\includegraphics[trim= 0cm 10cm 0cm 10cm,scale=0.6]{prob_dist_w-1.pdf}
\caption{p(x,y) for W=-1}
\end{figure}
\FloatBarrier
For $Y=WX$ and $P(W=1)=0.5=P(W=-1)$ we just need to combine these two cases to get the joint density of $X$ and $Y$.
\begin{figure}[htb]
\includegraphics[trim= 0cm 10cm 0cm 10cm,scale=0.6]{prob_dist.pdf}
...
\begin{flalign*}
	&= \exp \left( \ln \frac{1}{\sqrt{2 \pi \sigma^2}} \right) \exp \left( -\frac{x^2}{2\sigma^2} \right) \\
	&= \exp(-0.5 \ln(2 \pi \sigma^2)) \exp \left(-\frac{x^2}{2\sigma^2} \right) \\
	&= \exp \left( \frac{\ln(2\pi \sigma^2)}{4 \sigma^2} x^2 \right) \\
	&= \exp(-\lambda_2 x^2) \\
\end{flalign*}
Dann ist
